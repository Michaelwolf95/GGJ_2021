...
Description
  ...
  shadow volumes or ones that the camera will get especially close to. The HDRP and
  builtin versions of the shader do not suffer from this problem, and I'm be looking
  for a solution for it in the URP.
...
Quickstart Guide
  ...
  1) If using the URP or HDRP version, delete the BlobShadow.shader file and then 
     extract either the BlobShadowURP or BlobShadowHDRP unitypackage file.
  2) Create a new material asset.
  3) Set the shader of that material to be BlobShadow.
...
Material Properties Guide
  ...
  Rounded Cube Power - The power is can be thought of as the width of the inward push
    of the corners. The two default values (bias of 2.5, power of 3.5) for these should
